SELECT
	...
	LEFT JOIN         ndb.dslinks AS links   ON samples.datasetid = links.datasetid
	LEFT JOIN        ndb.datasets AS ds      ON samples.datasetid = ds.datasetid 
	LEFT JOIN           ndb.sites AS sts     ON links.siteid = sts.siteid
...
WHERE
	...
	(${siteid} IS NULL OR links.siteid IN (${siteid:csv})) AND

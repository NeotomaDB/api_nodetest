WITH dssites AS (
  SELECT
    ...
    FROM
      ...
      LEFT OUTER JOIN ndb.sitegeopolitical AS sgp ON sgp.siteid = sts.siteid
      LEFT OUTER JOIN            ndb.geopaths AS gp ON sgp.geopoliticalid = gp.geoin
      ...
      WHERE
        ${gpid} && gp.geoout OR sgp.geopoliticalid = ANY(${gpid})

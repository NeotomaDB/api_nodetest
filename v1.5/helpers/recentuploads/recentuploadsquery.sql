...
investigators AS (
	  SELECT investigator.datasetid,
           string_agg(investigator.name, ', ' ORDER BY investigator.piorder ASC) AS name
	FROM investigator
	GROUP BY investigator.datasetid
),	
geopolitical AS (
    SELECT ds.datasetid,
		       st.sitename,
...
geopoliticals AS (
    SELECT geopolitical.datasetid,
           string_agg(geopolitical.geoname, ' | ' ORDER BY geopolitical.rank ASC) AS geoname
	  FROM geopolitical
	  GROUP BY geopolitical.datasetid
)
...
WHERE ds.recdatecreated BETWEEN date_trunc('month', current_date - $1 * INTERVAL '1 month') AND date_trunc('day', current_date + INTERVAL '1 day')
